...
默认 **Ctrl+Alt+V**（macOS 为 **⌘⌥V**，V for Voice），触发语义与点击麦克风按钮**完全一致**：空闲时开始、录音中停止并冲刷尾段、启动中取消；键名不区分大小写。

- **改绑**：麦克风旁的 **⋮ 菜单** → 「快捷键」行，点击后显示「请按下快捷键…」，按下组合即绑定并持久化（刷新后仍生效）；行内 **✕** 恢复默认。
- **目标按焦点解析**：焦点在提问卡回答框 → 录向该卡；焦点在主输入框（含麦克风按钮本身）或页面空白 → 录向主输入框；焦点在设置页搜索框等其他可编辑控件 → **不触发、不劫持按键**。**停止录音不依赖焦点**：录音中只要焦点还在页面内，按快捷键即停止并冲刷尾段（焦点在 DevTools 或浏览器地址栏时任何网页都收不到按键）。
- **保留键当场拒绝**（Ctrl/⌘ + W/T/N/Tab/L、F11/F12、Alt+F4、只按修饰键）；**Space 系组合**（`Ctrl+Space`、`Ctrl+Shift+Space`）**允许绑定但会提示**可能被输入法/系统/浏览器扩展占用——`Ctrl+Shift+Space` 是 Chrome 翻译类扩展（Google Translate 等）的默认占位，会被吞掉导致"按了没反应"，因此默认键位已改为一组双修饰键 + 字母。
- 主键按**物理键位**匹配：macOS 的 Option 组合（`event.key` 会变成 `√` 等符号）与非美式键盘布局下同样可用。
- 输入法组合过程中的按键、按住不放的自动重复都不会触发；录音中按回车发送时麦克风自动停止（见下）。
...
**回车发送即停（与上面的开关无关）**：主输入框录音中按下不带 Shift 的 Enter 或 Ctrl/⌘+Enter（= 宿主的发送手势）会立即停止录音并**丢弃**未定稿尾段——消息已经发出，迟到的识别文本不应落进发送后新建的空草稿。`Shift+Enter`（换行）与提问卡回答框里的回车不触发。
...
两个过滤器模型（Silero VAD ~2.2MB、GTCRN <2MB）与主模型一样**不打包进 npm**，首次使用时自动后台下载；两者位于不同的官方发布标签（VAD 在 `asr-models`，GTCRN 在 `speech-enhancement-models`）。它们是增强组件：缺失或加载失败时自动旁路（fail-open），听写照常可用，`npm run doctor` 与 `/health` 会如实标注状态。

**存量资产自动补齐**：主模型已就绪但过滤器资产缺失或为 0 字节时（例如从 0.3.0 升级、GTCRN 曾因地址错配从未下载成功），插件会在下次录音时后台触发一次补下载；不重复下载主模型、不阻塞录音，补齐后无需重启即生效。

`/health` 的 `filters.vad` 暴露守门运行计数与最近 10 次判定值（`judgedRuns` / `emptyRuns` / `recentSpeechMs`）、`filters.denoiser` 暴露运行次数与最近错误，可一眼辨识“判定值恒为常数”或“组件每次都失败”这类静默失效；守门判空时 `/transcribe` 响应（无需开 debug）会附带 `gateReason`，客户端在同一会话内连续两段判空时给出一次可读提示，而不是静默无反应。
...
npm test             # 单元 + 路由集成测试
npm run test:e2e     # jsdom 浏览器端 e2e（快捷键/目标解析/回车自停/IME 延后）
node scripts/smoke.mjs [wav] [expected.txt]   # 真实模型慢速冒烟（无参默认用 test/fixtures/voice-zh-16k.wav）
```

慢速真模型 lane 也会在打 `v*` tag / 发布 Release 前由 `.github/workflows/smoke.yml` 自动触发，不拖累每次 PR。
