...
/**
 * Silero VAD 的窗口粒度（样本）。上游 `VoiceActivityDetector::AcceptWaveform`
 * 的契约是"每次约一个 window"（源码注释：*note n is usually window_size*、
 * *Please don't use a very large n*）——单次塞入大 buffer 会把语音起点锚定在
 * 缓冲区尾部，判定值塌缩为常数 `2*windowSize + minSpeechDuration`。
 * 故 corePipeline 必须按本粒度分块喂入（含尾部余数）。
 */
export const VAD_WINDOW_SIZE = 512;
/** `/health` 暴露的最近判定值环形缓冲长度。 */
export const RECENT_SPEECH_LIMIT = 10;
...
const VAD_CONFIG = {
  threshold: 0.5,
  windowSize: VAD_WINDOW_SIZE, // 与分块喂入粒度同源，防止二者漂移
...
export function createAudioFilter(deps = {}) {
  ...
  // 每组件独立留痕（评审 D5）：runs/errors/lastError 各自累计，一次性告警标志
  // 也按组件拆分——旧的全局 runtimeWarned 会让"任意组件抛过一次错"之后，
  // 另一个组件再抛错永不告警、且不留 message、不区分来源。
  const state = {
    vad: { instance: null, state: 'unloaded', runs: 0, errors: 0, lastError: null, warned: false },
    denoiser: { instance: null, state: 'unloaded', runs: 0, errors: 0, lastError: null, warned: false },
    degradedRuns: 0,
    judgedRuns: 0,
    emptyRuns: 0,
    recentSpeechMs: [],
  ...
  const COMPONENT_LABEL = { vad: 'Silero VAD', denoiser: 'GTCRN 降噪器' };

  /** 记录某组件的运行期异常：计数 + 最近错误 + 该组件独立的一次性告警。 */
  function recordComponentError(component, cause) {
    const slot = state[component];
    if (slot === undefined) return;
    slot.errors += 1;
    slot.lastError = String(cause?.message ?? cause);
    if (!slot.warned) {
      slot.warned = true;
      logger.warn?.(`[dsh-voice-local] ${COMPONENT_LABEL[component] ?? component} 推理异常，本段旁路原始音频:`, cause?.message ?? cause);
    }
  }

  /** 记录一次完成的判定：计数 + 最近判定值环形缓冲（只保留最近 N 次）。 */
  function rememberJudgment(speechMs) {
    state.judgedRuns += 1;
    const ring = state.recentSpeechMs;
    ring.push(speechMs);
    while (ring.length > RECENT_SPEECH_LIMIT) ring.shift();
  }
  ...
  /** 组件未就绪时的旁路原因（fail-open 也要说清是缺失还是加载/推理错误）。 */
  function bypassGateReason() {
    const s = state.vad.state;
    return s === 'error' || s === 'degraded' ? 'bypass-error' : 'bypass-missing';
  }
  ...
  async function corePipeline(samples, opts) {
    ...
    // 出错的组件归属：整段 try/catch 在 filterPipeline（运行时边界），这里用
    // stage 记录当前正在跑哪一步，catch 时据此把异常记到对应组件名下。
    let stage = null;
    let denoiseFailed = false;
    try {
      let out = samples;
      let denoised = false;
      if (denoiseEnabled) {
        ...
        if (den) {
          stage = 'denoiser';
          state.denoiser.runs += 1;
          out = runDenoise(den, out);
          denoised = out !== samples;
        }
        // 未生效（缺失/降级/无有效输出）都算 denoiseFailed，供 debug meta 区分。
        denoiseFailed = !denoised;
      }
      ...
      if (vadEnabled) {
        ...
        if (vad) {
          ...
          state.vad.runs += 1;
          vad.clear();
          // 上游契约：每次约一个 window（512 样本），含末尾不足一块的余数。
          // 单次整段喂入会把语音起点锚定在缓冲区尾部 → 判定值恒为常数（0.3.0 P0）。
          for (let offset = 0; offset < out.length; offset += VAD_WINDOW_SIZE) {
            vad.acceptWaveform(out.subarray(offset, Math.min(offset + VAD_WINDOW_SIZE, out.length)));
          }
          // 冲刷 VAD 内部状态，弹出尚未因尾静音而闭合的语音段（design D1 流程图）。
          // 不 flush 时，尾静音不足 minSilenceDuration 的短语音永远不闭合 → 判定 0ms，
          // 极短但真实的语音会被静默吞掉（实测 3s 切片 0ms vs flush 后 2074ms）。
          vad.flush();
          let voicedSamples = 0;
          while (!vad.isEmpty()) {
            const seg = vad.front();
          ...
          if (speechMs < minSpeechMs) {
            ...
            return {
              ...
              samples: out,
              bypass: false,
              gateReason: speechMs === 0 ? 'no-speech' : 'below-min-speech',
            };
          }
        }
      }
      ...
      const bypass = vadEnabled && !vadJudged;
      return {
        speech: true,
        speechMs,
        denoised,
        ...
        samples: out,
        bypass,
        gateReason: bypass ? bypassGateReason() : 'speech',
      };
    } catch (cause) {
      // 单组件异常：记到该组件名下（独立计数/告警），本段 fail-open 透传原样本。
      state.degradedRuns += 1;
      recordComponentError(stage ?? 'vad', cause);
      return {
        speech: true,
        speechMs: null,
        denoised: false,
        denoiseFailed: denoiseEnabled,
        samples,
        bypass: true,
        error: String(cause?.message ?? cause),
        gateReason: 'bypass-error',
      };
    }
  }
  ...
  async function filterPipeline(samples, opts = {}) {
    if (!(samples instanceof Float32Array) || samples.length === 0) {
      return {
        speech: true, bypass: true, reason: 'empty-input', speechMs: null,
        denoised: false, denoiseFailed: false, gateReason: 'speech', samples,
      };
    }
    const run = queueTail.then(async () => {
      try {
        ...
        // corePipeline 内部已按组件归属处理异常；这里是队列/编排层的兜底。
        state.degradedRuns += 1;
        recordComponentError('vad', cause);
        return {
          speech: true,
          speechMs: null,
          denoised: false,
          denoiseFailed: opts.denoiseEnabled !== false,
          samples,
          bypass: true,
          error: String(cause?.message ?? cause),
          gateReason: 'bypass-error',
  ...
  function status() {
    return {
      vad: {
        state: state.vad.state,
        runs: state.vad.runs,
        errors: state.vad.errors,
        lastError: state.vad.lastError,
      },
      denoiser: {
        state: state.denoiser.state,
        runs: state.denoiser.runs,
        errors: state.denoiser.errors,
        lastError: state.denoiser.lastError,
      },
      degradedRuns: state.degradedRuns,
      judgedRuns: state.judgedRuns,
      emptyRuns: state.emptyRuns,
      recentSpeechMs: [...state.recentSpeechMs],
    };
  }
  ...
  function dispose() {
    ...
    state.vad.runs = 0;
    state.vad.errors = 0;
    state.vad.lastError = null;
    state.vad.warned = false;
    state.denoiser.instance = null;
    state.denoiser.state = 'unloaded';
    state.denoiser.runs = 0;
    state.denoiser.errors = 0;
    state.denoiser.lastError = null;
    state.denoiser.warned = false;
    state.degradedRuns = 0;
    state.judgedRuns = 0;
    state.emptyRuns = 0;
    state.recentSpeechMs = [];
  }
