...
// ---- 噪声过滤器模型资产（add-voice-noise-filtering / add-voice-hotkey-and-fix-gating D3）----
// 与 SenseVoice 同源：sherpa-onnx GitHub release；同样不进 npm 包，首次需要时运行时下载
// （保持"模型不出包、镜像可达"的既有发布架构）。
// 各资产按**实际发布标签**分区：VAD 在 asr-models，GTCRN 在 speech-enhancement-models。
// 0.3.0 把 GTCRN 也指向 asr-models，该地址直连与镜像均 404，降噪能力从未真正生效过。
const FILTER_RELEASE_BASE = 'https://github.com/k2-fsa/sherpa-onnx/releases/download/';
...
export const FILTER_ASSETS = [
  {
    ...
    description: 'Silero VAD（约 2.2MB）',
  },
  {
    key: 'denoiser',
    filename: 'gtcrn_simple.onnx',
    subdir: 'denoiser',
    releaseTag: 'speech-enhancement-models',
    description: 'GTCRN 降噪（亚 2MB）',
  },
];

/**
 * 某资产的候选 URL 列表（主源 + 各镜像前缀），下载与失败诊断共用同一构造，
 * 保证诊断里列出的就是实际尝试过的地址。
 */
export function filterAssetUrls(asset) {
  const base = `${FILTER_RELEASE_BASE}${asset.releaseTag}/${asset.filename}`;
  return [base, ...FILTER_MIRROR_PREFIXES.map((prefix) => prefix + base)];
}
...
export async function downloadFilterAssets(options = {}) {
  ...
  for (const asset of FILTER_ASSETS) {
    ...
    const urls = filterAssetUrls(asset);
    // 逐条候选地址记录失败原因（含镜像），供状态接口直接定位标签/镜像错配。
    const attempts = [];
    let ok = false;
    for (const url of urls) {
      ...
      if (ok) break;
      attempts.push({ url, error: urlError instanceof Error ? urlError.message : String(urlError ?? '未知错误') });
    }
    results[asset.key] = ok
      ...
      : {
        ok: false,
        attempts,
        error: `全部候选地址均失败（${attempts.length} 条）：${attempts.map((a) => `${a.url} → ${a.error}`).join('；')}`,
      };
  }
  return results;
}
