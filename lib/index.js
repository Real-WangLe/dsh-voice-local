  ...
  async function filtersHealthSnapshot() {
    ...
    return {
      vad: {
        ...
        // 守门可观测面（design D2）：运行计数 + 最近判定的累计人声时长。
        // "判定结果恒为同一数值"这类失效可在此一眼辨识。
        runs: runtime.vad.runs,
        errors: runtime.vad.errors,
        lastError: runtime.vad.lastError,
        judgedRuns: runtime.judgedRuns,
        emptyRuns: runtime.emptyRuns,
        recentSpeechMs: runtime.recentSpeechMs,
      ...
      denoiser: {
        ...
        // 降噪纳入同一可观测面（评审 D5）：组件已加载但每次都失败时可被直接辨识。
        runs: runtime.denoiser.runs,
        errors: runtime.denoiser.errors,
        lastError: runtime.denoiser.lastError,
      },
    };
  }

  /** 本次判定的原因（spec「环境噪声人声守门」）：真实管线给出，seam 缺省按结果推导。 */
  function deriveGateReason(filtered) {
    if (typeof filtered?.gateReason === 'string') return filtered.gateReason;
    if (filtered?.speech === true) return 'speech';
    const ms = Number.isFinite(filtered?.speechMs) ? filtered.speechMs : 0;
    return ms > 0 ? 'below-min-speech' : 'no-speech';
  }
    ...
    try {
      ...
      if (req.method === 'POST' && suffix === '/transcribe') {
        ...
        try {
          ...
          filtered = {
            speech: true, samples: decoded.samples, bypass: true, speechMs: null,
            denoised: false, denoiseFailed: denoiseEnabled, gateReason: 'bypass-error',
          };
        }

        const gateReason = deriveGateReason(filtered);
        const metaBase = {
          guarded: false,
          denoised: filtered.denoised === true,
          denoiseFailed: filtered.denoiseFailed === true,
        ...
        if (filtered.speech !== true) {
          ...
          // OV-4 定案：守门判空对客户端可见——保持 { ok, text } 结构不变，
          // 无需开启调试开关即可区分"未检出人声 / 低于门限"。
          const payload = { ok: true, text: '', gateReason };
          if (debug) payload.meta = { ...metaBase, guarded: true };
